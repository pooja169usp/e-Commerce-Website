...
$(document).ready(function() {
	...
	function isCartEmpty() {
		var cartList = cart.getCartArray();
	        if(cartList.length == 0 || cart.size == 0) {
		        $('#showcart').html("<h1>Oops! Didn't add anything to your cart yet? </h1><br><h3>Here... Try our <a href='products.html'>Bertha's Deluxe Chocolates</a>");
				$('#order').hide();
				$('.displaycart').hide();
			}
	}
	...
	function showCartList(string) {
		...
		if(string=="good") {
			...
			$('#showcart').html("<h1 id='congrats'>Congratulations! Your order has been placed successfully. </h1><br><h3>Delicious and Yummilicious Bertha's Deluxe chocolates are on the way!</h3>");
			$('#order').hide();
			$('.displaycart').hide();
			displayCustomerDetails();
			document.cookie = 'jadrn017=; expires=-1;path=/'
			return;
		}
		if(string != "great") {
			if (cartList.length == 0 || cart.size == 0) {
	        ...
	        var tempStr = '<br /><ul><li><table class="displaycart" align="center"><tr><th>Products in your cart</th><th>Name</th><th>Quantity</th><th>Price</th><th>Total</th>';
	        ...
	        for(var i=0; i < cartList.length; i++) {
			 ...
			 var cost;
	          ...
	          for(var j=0; j < proj4_data.length; j++) {
	           ...
	           if(proj4_data[j][0] == cartList[i][0]) {   
	               cost = parseFloat(proj4_data[j][6]);
	               name = proj4_data[j][2];
	               var costProduct = quantity * cost;
	               total += costProduct;
				   
	               tempStr += "<tr><td><img src=\"/~jadrn000/PROJ4_IMAGES/"+proj4_data[j][0]+".jpg\" alt=\""
				   +proj4_data[i][2]+"\""+" width=\"200px\"  /></td><td><b>"+name+"</b><br><i>"+proj4_data[j][1]+"</i></td>";
					
					if(string=="good")
					tempStr += "<td><input type='text' name='quantity' size='5' class='"+proj4_data[j][0]+"' value='"+quantity+"' /></td>";
				   ...
				   if(string=="good")
					tempStr +="<td><input type='button' class='updateqty' id="+proj4_data[j][0]+" value='Update Quantity' /><br><br>"
				   +"<input type='button' class='delete' id="+proj4_data[j][0]+" value='Remove item' /></td></tr>";
